#clean zygoptera only ancestral state reconstruction
...
#prune tree to match data
chk_zygo<-name.check(zygoptera_tree, odonate_terr_data, data.names=as.character(odonate_terr_data$sn))
summary(chk_zygo)
...
zygo_terr_data <- odonate_terr_data[!(odonate_terr_data$sn %in% species_to_drop_zygo), ] #dropped data_not_tree species from dataset
name.check(zygo_tree, zygo_terr_data, data.names=as.character(zygo_terr_data$sn))
...
zygo_terr_mode <- factor(ifelse(zygo_terr_data$sp_terr == 1, "yes", "no"))
names(zygo_terr_mode) <- zygo_terr_data$sn

#plot this
cols<-setNames(c("turquoise", "brown"), levels(zygo_terr_mode)) #brown =yes territorail, turquoise = not territorial
