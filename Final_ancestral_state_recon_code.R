...
name_corrections <- c(
  "Acanthagrion_apical" = "Acanthagrion_apicale",
  "Anaciaeschna_isoceles" = "Anaciaeschna_isosceles",
  "Chalcolestes viridis" = "Chalcolestes_viridis"
)

my_data$Formatted_species <- ifelse(
  my_data$Formatted_species %in% names(name_corrections),
  name_corrections[my_data$Formatted_species],
  my_data$Formatted_species
)
...
chk<-name.check(odonate_tree, terr_data, data.names=as.character(terr_data$sn))
summary(chk)
odonate_tree<-drop.tip(odonate_tree, chk$tree_not_data) #dropped tree_not_data species
...
#prune tree to match data
chk<-name.check(odonate_tree, odonate_data, data.names=as.character(odonate_data$Genus_sp))
summary(chk)
odonate_tree<-drop.tip(odonate_tree, chk$tree_not_data) #dropped tree_not_data species
odonate_tree
...
#the node for the common ancestor of all odonates = 435, all Anisoptera = 436, all Zygoptera = 661
#plot(odonate_tree, show.node.label = TRUE)  # Plot tree with tip labels
#nodelabels() 

internal_node_index <- 435 - (length(fit_marginal$phy$tip.label))
...
anis_internal_node_index <- 436 - (length(fit_marginal$phy$tip.label))
...
zygo_internal_node_index <- 661 - (length(fit_marginal$phy$tip.label))
...
chk_ovi_size<-name.check(odonate_tree, combined_data, data.names=as.character(combined_data$Species))
summary(chk_ovi_size)
tree_ovi_size <- drop.tip(odonate_tree, chk_ovi_size$tree_not_data)#dropped tree_not_data species
...
#identify species to drop
chk_mate_guard<-name.check(odonate_tree, data_mate_guard_terr_old, data.names=as.character(data_mate_guard_terr_old$Species))
summary(chk_mate_guard)
tree_mate_guard<-drop.tip(odonate_tree, chk_mate_guard$tree_not_data) #dropped tree_not_data species
#identify species to drop from data
...
trait_data_filtered_no_courtship <- trait_data_filtered %>%
  dplyr::filter(Trait != "Prop_Courtship")

#plot
ggplot(trait_data_filtered_no_courtship, aes(x = Trait, y = Species, fill = Value)) +
...
chk_terr<-name.check(odonate_tree, odonate_terr_data, data.names=as.character(odonate_terr_data$sn))

#chk_terr is not returning a list like before, so we do this:
if (!is.character(chk_terr)) {
  # name.check found mismatches → prune accordingly
  tree_terr_signal <- drop.tip(odonate_tree, chk_terr$tree_not_data)
  terr_signal_species_to_drop <- chk_terr$data_not_tree
  terr_signal_data <- odonate_terr_data[!(odonate_terr_data$sn %in% terr_signal_species_to_drop),]
} else {
  # name.check returned "OK" → nothing to prune
  tree_terr_signal <- odonate_tree
  terr_signal_data <- odonate_terr_data
}
...
chk_fly_v_perch<-name.check(odonate_tree, binary_fly_v_perch_df, data.names=as.character(binary_fly_v_perch_df$sn))
tree_fly_v_perch <- drop.tip(odonate_tree, chk_fly_v_perch$tree_not_data)#dropped tree_not_data species
#identify species to drop from data
...
chk_court<-name.check(odonate_tree, binary_court_df, data.names=as.character(binary_court_df$sn))
tree_court <- drop.tip(odonate_tree, chk_court$tree_not_data)#dropped tree_not_data species
#identify species to drop from data
...
chk_ovi<-name.check(odonate_tree, binary_ovi_df, data.names=as.character(binary_ovi_df$sn))
tree_ovi <- drop.tip(odonate_tree, chk_ovi$tree_not_data)#dropped tree_not_data species
#identify species to drop from data
...
chk_lo_len<-name.check(odonate_tree, binary_lo_len, data.names=as.character(binary_lo_len$sn))
tree_lo_len <- drop.tip(odonate_tree, chk_lo_len$tree_not_data)#dropped tree_not_data species
#identify species to drop from data
...
#prune tree to match data
chk<-name.check(odonate_tree, mate_guard_df, data.names=as.character(mate_guard_df$sn))
summary(chk)
odonate_tree<-drop.tip(odonate_tree, chk$tree_not_data) #dropped tree_not_data species
odonate_tree
# Identify species to drop from terr_data
...
#prune tree to match data
chk_hab_size<-name.check(odonate_tree, combined_data, data.names=as.character(combined_data$sn))
summary(chk_hab_size)
tree_hab_size <- drop.tip(odonate_tree, chk_hab_size$tree_not_data)
#identify species to drop from data
