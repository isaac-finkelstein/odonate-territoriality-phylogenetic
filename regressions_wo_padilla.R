...
wing_data<- read.csv("data/data_without_Padilla-Morales and Waller copy.csv") #same dataset as old one (V3) but added wing pigment and climate data
...
rownames(odonate_data_for_male_wing_reg)<-odonate_data_for_male_wing_reg$sn
...
mod_male_wing_pig<-phyloglm(male_wing_pigment~mean_annual_temperature, data = odonate_data_for_male_wing_reg, phy=odonate_tree_first_reg, boot=1000, method = 'logistic_MPLE', btol = 10)
summary(mod_male_wing_pig)
...
#prune tree to the data for this regression only
data_for_interaction<-merge(odonate_data_for_male_wing_reg, terr_data, by="sn", all=TRUE)
data_for_interaction <- na.omit(data_for_interaction)
chk_int_reg<-name.check(tree, data_for_interaction, data.names = as.character(data_for_interaction$sn))
summary(chk_int_reg)
odonate_tree_int_reg<-drop.tip(tree, chk_int_reg$tree_not_data)
species_to_drop_int_reg<-chk_int_reg$data_not_tree
odo_data_for_interaction<-data_for_interaction[!(data_for_interaction$sn %in% species_to_drop_int_reg), ]
rownames(odo_data_for_interaction)<-odo_data_for_interaction$sn
name.check(odonate_tree_int_reg, odo_data_for_interaction, data.names=as.character(odo_data_for_interaction$sn))
str(odo_data_for_interaction)

mod_male_wing_pig<-phyloglm(male_wing_pigment~mean_annual_temperature*terr, data = odo_data_for_interaction, phy=odonate_tree_int_reg, boot=1000, method = 'logistic_MPLE', btol = 35)
summary(mod_male_wing_pig)
#remember that because I have a binary response variable, I can't just interpret the p-values of the interaction term outright. 
#dig into what btol I should set. 

#Next, let's do the regression testing territorialiy on sexual wing dichromatism
#Hypothesis 3: Territoriality is a causal factor in the evolution of sexual wing dichromatism
third_hypo_data<-merge(terr_data, wing_dichro_var_df, by = "sn", all=TRUE)
third_hypo_data<-na.omit(third_hypo_data)
chk_third_reg<-name.check(tree, third_hypo_data, data.names = as.character(third_hypo_data$sn))
summary(chk_third_reg)
tree_third_reg<-drop.tip(tree, chk_third_reg$tree_not_data)
species_to_drop_third_reg<-chk_third_reg$data_not_tree
odo_third_hypo_data<-third_hypo_data[!(third_hypo_data$sn %in% species_to_drop_third_reg), ]
rownames(odo_third_hypo_data)<-odo_third_hypo_data$sn
name.check(tree_third_reg, odo_third_hypo_data, data.names=as.character(odo_third_hypo_data$sn))
str(odo_third_hypo_data)

third_hypo<-phyloglm(wing_dichromatism~terr, data = odo_third_hypo_data, phy=tree_third_reg, boot=1000, method = 'logistic_MPLE', btol = 10)
summary(third_hypo)
#may want to dig into why the btol is set to 35 and what I should set it to.
#it seems that btol is a prior - so you "bound the searching space" 
#btol of 10 gives a p-value of 0.2, btol of 35 gives a p-value of 0.97
#so the btol changes the results a lot!


#for hypotheses 4 and 5, I'm not sure how to isolate for female-biased/male biased sexual dichromatism
#for now, I am just doing an interaction between climate and territoriality on sexual dichromatism
fourth_hypo_data<-merge(terr_data, wing_dichro_var_df, by = "sn", all=TRUE)
fourth_hypo_data<-merge(fourth_hypo_data, climate_var_df, by="sn", all=TRUE)
fourth_hypo_data<-na.omit(fourth_hypo_data)
chk_fourth_reg<-name.check(tree, fourth_hypo_data, data.names=as.character(fourth_hypo_data$sn))
summary(chk_fourth_reg)
tree_fourth_reg<-drop.tip(tree, chk_fourth_reg$tree_not_data)
species_to_drop_fourth_reg<-chk_fourth_reg$data_not_tree
odo_fourth_hypo_data<-fourth_hypo_data[!(fourth_hypo_data$sn %in% species_to_drop_fourth_reg), ]
rownames(odo_fourth_hypo_data)<-odo_fourth_hypo_data$sn
name.check(tree_fourth_reg, odo_fourth_hypo_data, data.names=as.character(odo_fourth_hypo_data$sn))
str(odo_fourth_hypo_data)

fourth_hypo<-phyloglm(wing_dichromatism~terr*mean_annual_temperature, data=odo_fourth_hypo_data, phy = tree_fourth_reg, boot=1000, method='logistic_MPLE', btol=35)
summary(fourth_hypo)
#sample size is only 60 (for initial regression)
#btol of 35 makes lower p-values than a btol of 10 -- but insiginificant either way.